...
\subsection{Theta}
Theta $ \frac{d V}{d t} $ measures the instantaneous sensitivity of option prices against time. Given the definition that $ t = T^* - \tau $.

\subsection{Delta}
Delta $ \frac{d V}{d F} $ measures the instantaneous sensitivity of option price against forward.

\subsection{Gamma}
Gamma $ \frac{d^2 V}{d F^2} $ is a second order derivative of option price against forward or the first derivaitive of delta against forward.
