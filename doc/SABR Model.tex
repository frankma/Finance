...
This parametrized model not only produces smooth volatility smile but also provides sensible model implied volatilities when the forward is shocked.
A crucial drawback of the model, however, is the lacking of evolution from one expiry to the next hence restrained from path dependent instruments pricing.
...
Under special case when $ \beta = 1 $, the underlying is modeled as Log-Normal process and the approximation could be reduced to the below.
